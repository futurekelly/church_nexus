export { KanbanBoard } from "./components/kanban-board";
export { VisitorCard } from "./components/visitor-card";
export { VisitorForm } from "./components/visitor-form";
export { ContactHistoryTimeline } from "./components/contact-history-timeline";
export { LogInteractionModal } from "./components/log-interaction-modal";

export { useFollowUp } from "./hooks/use-follow-up";

export * from "./types/follow-up.types";
export * from "./data/mock-visitors";
