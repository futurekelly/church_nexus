...
def validate_thumbnail(file):
    ...
    msg = "Invalid thumbnail file format."
...
def validate_audio(file):
    ...
    msg = "Invalid audio file format. Allowed formats: MP3, WAV, OGG."
    if content_type and content_type not in allowed_types:
        raise ValidationError(msg)
    if ext not in allowed_exts:
        raise ValidationError(msg)
...
def validate_video(file):
    ...
    msg = "Invalid video file format. Allowed formats: MP4, WEBM, OGG, MOV."
    if content_type and content_type not in allowed_types:
        raise ValidationError(msg)
    if ext not in allowed_exts:
        raise ValidationError(msg)
